...
Online Workspaceは、LiveKitを使用したリアルタイムビデオ通話機能を搭載した作業勉強スペースを提供するアプリケーションです。複数のユーザーが同じルームで勉強しながら、ポモドーロタイマーを共有したりチャットなどができます。
自分でルームのタイトルを決めてルームを作成したり参加することができます。
...
### 🚪 ログイン不要の即入室設計
Firebase Anonymous Authentication を採用し、アカウント作成なしで名前とルームタイトルを入力するだけで即座に入室できます。「気軽に集まれる」オンライン作業空間を最優先に設計しています。ユーザーIDはセッション内でFirestoreと同期されるため、参加者管理はシームレスです。

### 🏗️ Feature-Based Architecture
機能ごとに独立したミニモジュール（`src/features/`）として設計。各featureは `components/`（UI専用）・`hooks/`（ロジック専用）・`constants.js` に明確に分離されており、1ファイル200行以下を厳守しています。UIとロジックを分けることで、保守・拡張が容易になっています。

### ⚡ リアルタイム同期設計
- **Firestore `onSnapshot`**: ルーム一覧・タイマー状態・参加者リストをリアルタイムに全端末へ同期
- **WebSocket（ws）**: ゲームループなど低遅延が必要な通信はWebSocketで処理
- **LiveKit（WebRTC）**: 映像・音声は専用インフラにオフロードすることで、Firestoreへの負荷集中を回避

### 👑 ホスト権限システム
最初に入室したユーザーが自動的にホストとなり、共有ポモドーロタイマーの開始/停止やゲームの起動権限を持ちます。ホストが退室した場合でも次の参加者が権限を引き継ぐことができるよう設計しています（引き継ぎ機能は今後対応予定）。

### 🔒 セキュリティ設計
LiveKitのAPI Key/SecretやSlackのBot Tokenなどの機密情報はフロントエンドの `.env` に置かず、**Firebase Secret Manager** および **Cloud Functions** 経由で管理します。フロントエンドはURLとチャンネルIDのみを保持し、トークンが漏洩しない設計にしています。

### 🚩 機能フラグによる安全な段階リリース
Slack連携などオプション機能は `VITE_SLACK_FEATURE_ENABLED` のような環境変数フラグで制御します。フラグOFFのときはUIごと非表示になり、設定値が不完全な場合でもアプリが壊れないsafe no-op（安全な無操作）設計を採用しています。

### 📦 コード分割・最適化
Viteの `manualChunks` 設定でFirebase・LiveKit・React・UIライブラリを別バンドルに分離し、初期ロード時間を短縮。各ページは `React.lazy()` + `Suspense` でオンデマンドロードします。
